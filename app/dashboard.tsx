...
import { riskPhones } from '@/lib/risk-phones';
        ...
        {view === 'referrals' && (
          <>
            ...
            <section className="panel risk-roster" id="risk-student-list">
              ...
              <div className="risk-student-table"><Table><TableHeader><TableRow><TableHead>Оқушы</TableHead><TableHead>Телефон</TableHead><TableHead>Ағым</TableHead><TableHead>Куратор</TableHead><TableHead>Нәтиже</TableHead><TableHead><span className="sr-only">Әрекет</span></TableHead></TableRow></TableHeader><TableBody>{selectedRiskStudents.map((student, index) => { const group = groups.find((item) => item.name === student.curator && item.stream.endsWith(student.stream === 'Тамыз' ? 'aug' : 'sep')); const lastEntry = referrals.find((entry) => entry.title === student.name && entry.groupId === group?.id && entry.method.startsWith(riskSpecialist + ' · ')); const phone = riskPhones[student.name]; return <TableRow key={`${student.name}-${student.curator}-${index}`}><TableCell>{student.name}</TableCell><TableCell>{phone ? <span className="phone-cell"><span>{`+${phone.slice(0, 1)} (${phone.slice(1, 4)}) ${phone.slice(4)}`}</span><a className="whatsapp-link" href={`https://wa.me/${phone}`} target="_blank" rel="noreferrer">WhatsApp</a></span> : <span className="muted">—</span>}</TableCell><TableCell>{student.stream}</TableCell><TableCell>{student.curator}</TableCell><TableCell>{lastEntry ? <span className={'status status-' + lastEntry.status}>{conversationStatuses.find((item) => item.value === lastEntry.status)?.label}</span> : <span className="muted">Сөйлеспеді</span>}</TableCell><TableCell><button className="referral-button" disabled={!group} onClick={() => { if (!group) return; setGroupId(group.id); setReferralStudentId(''); setRiskConversation(student); setReferralTarget(`${riskSpecialist} · ${specialists.find(([name]) => name === riskSpecialist)?.[1] || ''}`); setStatus(lastEntry?.status || 'contact'); setModal('referral'); }}>{lastEntry ? 'Нәтижені өзгерту' : 'Сөйлесу нәтижесі'}</button></TableCell></TableRow>})}</TableBody></Table></div>
            </section>
          </>
        )}
        {view === 'cases' && (
