...
export function allowedGroups(user:Awaited<ReturnType<typeof identity>>){return user.role==='admin'||user.role==='specialist'||user.role==='conversation'?baseline:baseline.filter(group=>group.stream.endsWith(`-${user.month}`))}
...
export function sameOrigin(req:Request){
 ...
 const trustedProxyOrigins=['https://ms-retention-vercel-proxy.vercel.app'];
 return !origin||origin===new URL(req.url).origin||trustedProxyOrigins.includes(origin);
}
